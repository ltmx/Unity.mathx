...
namespace Unity.Mathematics
{
	public static partial class mathx
	{
		...
		static uint _rngState = 0x6E624EB7u;

		[MI(IL)] static ref Random globalRng() => ref Unsafe.As<uint, Random>(ref _rngState);
		...
		[MI(IL)] public static Random setseed(this ref Random rand, float seed)
		{
			rand.state = seed.hash();
			return rand;
		}
		[MI(IL)] public static Random setseed(this ref Random rand, float2 seed)
		{
			rand.state = seed.hash();
			return rand;
		}
		[MI(IL)] public static Random setseed(this ref Random rand, float3 seed)
		{
			rand.state = seed.hash();
			return rand;
		}
		[MI(IL)] public static Random setseed(this ref Random rand, float4 seed)
		{
			rand.state = seed.hash();
			return rand;
		}
