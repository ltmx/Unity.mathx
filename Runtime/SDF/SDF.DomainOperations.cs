...
namespace Unity.Mathematics
{
	...
	public static partial class SDF
	{
		...
		/// scaling factors it.
		static float3 opTransformx(this float3 p, float3x4 t) => p.mul(t.inverse()).xyz;

		// Scale -----------------------------------------------------------------------------------------------

		/// Scale Operation
		static float3 opScale(this float3 p, float s) => p * s;
		...
		/// Symmetry in Y axis
		static float3 opSymX(this float3 p) => f3(p.x.abs(), p.yz);
		/// Symmetry on Y axis
		static float3 opSymY(this float3 p) => f3(p.x, p.y.abs(), p.z);
		/// Symmetry on Z axis
		static float3 opSymZ(this float3 p) => f3(p.xy, p.z.abs());
		/// Symmetry on X and Y axis
		static float3 opSymXY(this float3 p) => f3(p.xy.abs(), p.z);
		/// Symmetry on X and Z axis
		static float3 opSymYZ(this float3 p) => f3(p.x, p.yz.abs());
		/// Symmetry on X and Z axis
		static float3 opSymXZ(this float3 p) => f3(p.x.abs(), p.y, p.z.abs());
		...
		static float3 opRep(this float3 p, float3 c) => (p + 0.5f * c).mod(c) - 0.5f * c;
		...
		static float3 opRepLim(this float3 p, float c, float3 l) => p - c * (p / c).round().clamp(-l, l);
		...
		static float3 opDisplace(this float3 p, float3 displacement) => p + displacement;

		// Twist --------------------------------------------------------------------------------------

		// todo : add params such as the axis of twist
		static float3 opTwist(float3 p, float twist)
		{
			sincos(twist * p.y, out float s, out float c);
			float2x2 m = f2x2(c, -s, s, c);
			float3 q = f3(m.mul(p.xz), p.y);
			return q;
		}

		// Bend ---------------------------------------------------------------------------------------

		static float3 opCheapBend(this float3 p, float bend)
		{
			sincos(bend * p.x, out float s, out float c);
			float2x2 m = f2x2(c, -s, s, c);
			float3 q = f3(m.mul(p.xy), p.z);
			return q;
		}
		...
		{
			float2 c = f2(p.xz.length(), p.y);
			float a = p.z.atan2(p.x);
			sincos(a, out float sin, out float cos);
