...
namespace Unity.Mathematics
{
	public static partial class SDF
	{
		...
		static float opIntersection(this float d1, float d2) => d1.max(d2);

		/// Subtract a shape from another. This is not commutative, so the order of the operands matters.
		static float opSubtraction(this float d1, float d2) => (-d1).max(d2);

		/// Adds two SDFs together. (The union of two SDFs is the minimum of the two)
		static float opUnion(this float d1, float d2) => d1.min(d2);
		...
		/// <returns></returns>
		static float opSmoothUnion(this float d1, float d2, float k) => k.smax(d1, d2);
		...
		static float opSmoothSubtraction(this float d1, float d2, float k) => k.smin(d1, -d2); //Todo : Check correctness of this ... Seems odd
		...
		/// <param name="k">Size of the transition</param>
		/// <returns></returns>
		static float opSmoothIntersection(float d1, float d2, float k) => k.smax(d1, d2);
	}
}