	...
	public static partial class mathx
	{
		...
		public static double GammaLn(double z)
		{
			if (z < 0.5)
			{
				...
				return LnPI - (PI * z).sin().ln() - s.ln() - LogTwoSqrtEOverPI - (0.5 - z) * ((0.5 - z + GammaR) / E).ln();
			}
			else
			{
				double s = GammaDk[0];
				...
				return s.ln() + LogTwoSqrtEOverPI + (z - 0.5) * ((z - 0.5 + GammaR) / E).ln();
			}
		}
		...
		public static double Gamma(double z)
		{
			if (z < 0.5)
			{
				...
				return PI / ((PI * z).sin() * s * TwoSqrtEOverPI * ((0.5 - z + GammaR) / E).pow(0.5 - z));
			}
			else
			{
				double s = GammaDk[0];
