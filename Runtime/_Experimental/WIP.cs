...
namespace Unity.Mathematics
{
	...
	public static class RandomUtils
	{
		...
		static readonly uint4 PRIMEX = new(2654435761u, 2246822519u, 3266489917u, 668265263u);

		public static float normalizeuint(this uint value) => value / (float)uint.MaxValue;

		static float randf(int seed) => xxhash32((uint)seed).normalizeuint();
		static float2 randf(int2 seed) => new();
		static float3 randf(int3 seed) => new(randf(seed.x), randf(seed.y), randf(seed.z));
		static float4 randf(int4 seed) => new(randf(seed.x), randf(seed.y), randf(seed.z), randf(seed.w));

		static float rand(uint seed) => xxhash32(seed) / (float)uint.MaxValue;
		...
		static uint xxhash32(uint seed)
		{
