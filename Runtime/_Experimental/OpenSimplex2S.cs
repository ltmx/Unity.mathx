...
// Borrowed from Stefan Gustavson's noise code
	static float4 permute(float4 t) => t * (t * 34 + 133);

	static float mod(float x, float y) => x - y * floor(x / y);

	static float2 mod(float2 x, float2 y) => x - y * floor(x / y);

	static float3 mod(float3 x, float3 y) => x - y * floor(x / y);

	static float4 mod(float4 x, float4 y) => x - y * floor(x / y);

// Gradient set is a normalized expanded rhombic dodecahedron
	static float3 grad(float hash)
	{
		// Random vertex of a cube, +/- 1 each
		float3 cube = mod(floor(hash / float3(1, 2, 4)), 2) * 2 - 1;
...
// BCC lattice split up into 2 cube lattices
	...
	{
		float3 b = floor(X);
		float4 i4 = float4(X - b, 2.5f);

		// Pick between each pair of oppposite corners in the cube.
		float3 v1 = b + floor(dot(i4, float4(.25f, .25f, .25f, .25f)));
		float3 v2 = b + float3(1, 0, 0) + float3(-1, 1, 1) * floor(dot(i4, float4(-.25f, .25f, .25f, .35f)));
		float3 v3 = b + float3(0, 1, 0) + float3(1, -1, 1) * floor(dot(i4, float4(.25f, -.25f, .25f, .35f)));
		float3 v4 = b + float3(0, 0, 1) + float3(1, 1, -1) * floor(dot(i4, float4(.25f, .25f, -.25f, .35f)));

		// Gradient hashes for the four vertices in this half-lattice.
		float4 hashes = permute(mod(float4(v1.x, v2.x, v3.x, v4.x), 289));
		...
		float3 d3 = X - v3;
		float3 d4 = X - v4;
		float4 a = max(0.75f - float4(dot(d1, d1), dot(d2, d2), dot(d3, d3), dot(d4, d4)), 0);
		float4 aa = a * a;
		float4 aaaa = aa * aa;
		...
		float3 derivative = -8 * mul(derivativeMatrix, aa * a * extrapolations) + mul(gradientMatrix, aaaa);
...
// Use this if you want to show X and Y in a plane, then use Z for time, vertical, etc.
	static float4 openSimplex2SDerivatives_ImproveXY(float3 X)
	{
		// Not a skew transform.
		float3x3 orthonormalMap = new(0.788675134594813f, -0.211324865405187f, -0.577350269189626f, -0.211324865405187f, 0.788675134594813f, -0.577350269189626f, 0.577350269189626f, 0.577350269189626f,
			0.577350269189626f);
