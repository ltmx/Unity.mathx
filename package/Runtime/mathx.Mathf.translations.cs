	...
	public static partial class mathx
	{
		...
		[MI(IL)] public static float3 SampleParabola(float3 start, float3 end, float height, float t)
		{
			if ((start.y - end.y).abs() < 0.1f)
			{
				...
				float3 travelDirection = end - start;
				float3 result = start + t * travelDirection;
			...
			else
			{
				...
				float3 travelDirection = end - start;
				float3 levelDirection = end - new float3(start.x, end.y, start.z);
				float3 right = travelDirection.cross(levelDirection);
				float3 up = right.cross(travelDirection);
