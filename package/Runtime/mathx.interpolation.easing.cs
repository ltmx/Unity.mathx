...
namespace Unity.Mathematics
{
	public static partial class mathx
	{
		...
		[MI(IL)] public static float easeInOutCirc(this float x) => x < 0.5 ? (1 - sqrt(1 - (2 * x).sq())) * 0.5f : (sqrt(1 - (-2 * x + 2).sq()) + 1) * 0.5f;
		...
		[MI(IL)] public static float easeInOutBack(this float x)
		{
			...
			return x < 0.5 ? (2 * x).sq() * ((c2 + 1) * 2 * x - c2) * 0.5f : ((2 * x - 2).sq() * ((c2 + 1) * (x * 2 - 2) + c2) + 2) * 0.5f;
		}
		...
		[MI(IL)] public static float easeInOutBounce(this float x) => x < 0.5f ? (1 - easeOutBounce(1 - 2 * x)) / 2 : (1 + easeOutBounce(2 * x - 1)) / 2;
	}
}